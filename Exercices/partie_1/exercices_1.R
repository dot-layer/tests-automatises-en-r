...
# 2. Exécuter les tests. Les deux premiers passent déjà.
valeur_attendue_vecteur_1 <- prod(vecteur_1)^(1 / length(vecteur_1))
valeur_attendue_vecteur_2 <- prod(vecteur_2)^(1 / length(vecteur_2))

tryCatch(
  {
    comparaison <- all.equal(moyenne_geo(vecteur_1), valeur_attendue_vecteur_1, tolerance = 1e-5)
...
tryCatch(
  {
    comparaison <- all.equal(moyenne_geo(vecteur_2), valeur_attendue_vecteur_2, tolerance = 1e-5)
    if(!isTRUE(comparaison)) {
...
# 3. On désire gérer les nombres négatifs. Coder cette solution à l'aide de la formule dans la 
#    diapositive. Le test 3 passe.
valeur_attendue_vecteur_3 <- -2.352158
tryCatch(
    {
        comparaison <- all.equal(moyenne_geo(vecteur_3), valeur_attendue_vecteur_3, tolerance = 1e-5)
        if(!isTRUE(comparaison)) {
...
tryCatch(
    {
        comparaison <- all.equal(moyenne_geo(vecteur_4), valeur_attendue_vecteur_4, tolerance = 1e-5)
        if(!isTRUE(comparaison)) {
